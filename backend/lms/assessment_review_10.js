					...
					if(answer){
						...
						$.each(answer,function(key,value){
							...
							if(value.type=="multiple_choice"||value.type=="multiple_answer"){
								...
								$.each(short_answer_correct_array,function(key,value){
									trimmed_correct_array.push(value.trim());
...
$(document).on("click",".consider_answer",function(){
	// alert("answer considered yeah");
	if(confirm("Are you sure you want to consider this answer? You can remove it on the assessment editor.")){

		var considered_answer = $(this).parent().find("input").val();
		var considered_answer_order = $(this).attr("answer_key_order");
		var the_this = $(this);

		$.ajax({
		    url: base_url+"consider_answer/",
		    type: "POST",
		    data: {
		    	assessment_id:assessment_id,
		    	considered_answer:considered_answer,
		    	considered_answer_order:considered_answer_order,
		    },
		    // contentType: "application/json",
		    complete: function(response){
		    	...
		    	if(response.responseText=="true"){
		    		alert(considered_answer+" is now considered a correct answer.");
		    		$(the_this).parent().css("background-color","rgb(114, 196, 114)");
		    		$(the_this).hide();
