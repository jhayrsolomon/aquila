...
class Mailsmsconf
{
    ...
    public function mailsms($send_for, $sender_details, $date = null, $exam_schedule_array = null)
    {
        ...
        if (!empty($chk_mail_sms)) {
            ...
            elseif ($send_for == "lesson_assigned") {

                $this->console_log($chk_mail_sms['template']);

                if ($chk_mail_sms['mail'] && $chk_mail_sms['template'] != "") {
                    return $this->CI->mailgateway->send_lesson_details($sender_details['student_id'], $sender_details['email'], $chk_mail_sms['template'],$sender_details);
                }
            }
            ...
            elseif ($send_for == "parent_notification") {
