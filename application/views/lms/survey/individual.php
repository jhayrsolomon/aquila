	      	...
	      	<div class = "row row-height">
		        ...
		        <div class="col-sm-5 right">
		            ...
		            <div class="info col-sm-5">
		        		...
		        		<div class="info-row">
			        		<div class="info-tab col-sm-12" style="background-color: red;color: white;height: 30px;">
			        			<center>Close</center>
			        		</div>
		        		</div>
		        		<style type="text/css">
		        		...
		        		<div class="info-row print" style="background-color: green;cursor: pointer;">
...
<script type="text/javascript">
	...
	var answer_json = '<?php echo $respond; ?>';
	var final_json = {};
	...
	function populate_key(option_type,answer=""){
	...
	$(document).ready(function(){
		...
		if(stored_json){
			$.each(JSON.parse(stored_json),function(key,value){
				...
				var answer = JSON.parse(answer_json)[key].answer.split(",");
				...
				$.each(answer,function(answer_key,answer_value){
					if(value.type=="multiple_choice"||value.type=="multiple_answer"){
						if(answer_value=="1"){
							var last_option = $(".option-container-actual").eq(key).find(".option").eq(answer_key).find("input").eq(0).prop("checked",true);
							
						}
						$(".option-container-actual").eq(key).find(".option").eq(answer_key).find("input").eq(0).attr("disabled","");
					}
